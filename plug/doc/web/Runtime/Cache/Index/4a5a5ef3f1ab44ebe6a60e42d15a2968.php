            ...
            <div class="manual-head">
                <div class="left">
                   ...
                   <!--<a href="javascript:;" style="float: right;" onclick="pop('测试','<?php echo U('Doc/test',array('id'=>$get['id']));?>')">测试</a>-->
                    <?php if(!empty($auth)): ?><a href="<?php echo U('User/index');?>" style="float: right">账号管理</a><?php endif; ?>
                    <a href="<?php echo U('Login/logout');?>" style="float: right">注销</a>
                    <span style="float: right;margin-right: 10px"><?php echo ($username); ?></span>
...
<ul class="jstree-container-ul jstree-children jstree-wholerow-ul jstree-no-dots">
	...
	<?php if(is_array($cate)): foreach($cate as $key=>$cate): ?><li class="jstree-node">
			...
			<a class="jstree-anchor" id="cate_<?php echo ($cate["id"]); ?>" style="padding-left: 25px;width:270px">
				...
				<?php if(!empty($auth)): ?><font style="color:blue" onclick="pop('修改 <?php echo ($cate["title"]); ?>','<?php echo U('Doc/cate',array('id'=>$cate['id']));?>');">修改</font><?php endif; ?>
			...
			<ul class="jstree-children x-ul">
				<?php if(is_array($cate["child"])): foreach($cate["child"] as $key=>$child): ?><a class="jstree-node x_a" id="cate_<?php echo ($child["id"]); ?>">
						...
						<?php if(!empty($auth)): ?><font style="color:blue"  onclick="pop('修改 <?php echo ($child["title"]); ?>','<?php echo U('Doc/cate',array('id'=>$child['id']));?>');">修改</font><?php endif; ?>
	...
	<i class="jstree-themeicon">◆</i>添加
		...
		<?php if(!empty($api["syno"])): ?><h3 class="xq_title">接口介绍</h3>
                        ...
                        <?php if(!empty($api["result"])): ?><h3 class="xq_title">返回例子</h3>

                            <div class="view-body think-editor-content;">
                                <?php echo (htmlspecialchars_decode($api["result"])); ?>
                            </div><?php endif; ?>
